...
var ToDoList = React.createContext({
  ...
  DragEnd: function(e) {
    ...
    Array.prototype.forEach.call(this.dragged.parentNode.childNodes, function(
      node
    ) {
  ...
  render: function() {
    ...
    if (queryText) {
      ...
      return (
        <ul onDragOver={this.DragOver}>{queryResult.map(createItem, this)}</ul>
      );
    }
...
var ToDoListItem = React.createContext({
  ...
  render: function() {
    ...
    if (!this.props.children.isDone) _style = "none";
    return (
      <li
      ...
      >
        ...
        <input
          ...
          defaultChecked={this.props.children.isDone}
        />
        <span style={{ textDecoration: _style }}>
...
var ToDoFilter = React.createContext({
  isActive: function(value) {
    return (
      "btn " +
      (value === this.props.filter[0].Status ? "btn-primary" : "default")
    );
  },
  render: function() {
        ...
        <div className="col-xs-5">
          <ul className="nav nav-pills todo-filter">
            <li>
              <a
                onClick={onFilter1}
                className={this.isActive("SHOW_ALL")}
                value="SHOW_ALL"
                href="#"
              >
                All
      ...
      Todo: [
        {
          name: "parimary",
          items: [
            { item: "Todo itme #1", isDone: false },
            { item: "Todo itme #2", isDone: true },
